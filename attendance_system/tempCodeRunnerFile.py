...
attendance_df = pd.DataFrame(columns=["Name", "Attendance"])
for name in known_face_names:
    attendance_df = attendance_df.append({"Name": name, "Attendance": "Absent"}, ignore_index=True)
...
def update_attendance():
    ...
    for face_encoding, (top, right, bottom, left) in zip(face_encodings, face_locations):
        ...
        if face_distances[best_match_index] < 0.6:
            ...
            if left < (cap.get(3) // 2) and not var_list[best_match_index].get():
                # Person is entering from left side
                attendance_df.loc[attendance_df["Name"] == name, "Attendance"] = "Present"
                ...
                # Person is exiting from right side
                attendance_df.loc[attendance_df["Name"] == name, "Attendance"] = "Currently out"
    ...
    for i, var in enumerate(var_list):
        if var.get() == True:
            ...
            if attendance_df.loc[attendance_df["Name"] == name, "Attendance"].iloc[0] == "Absent":
                attendance_df.loc[attendance_df["Name"] == name, "Attendance"] = "Present"
                print(f"{name} is present.")  # Print attendance record
                checkboxes[i].config(bg="light green")  # Set background to light green
        else:
            name = known_face_names[i]
            if attendance_df.loc[attendance_df["Name"] == name, "Attendance"].iloc[0] == "Present":
                attendance_df.loc[attendance_df["Name"] == name, "Attendance"] = "Absent"
